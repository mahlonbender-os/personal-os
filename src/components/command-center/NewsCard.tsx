...
import { Newspaper, RefreshCw, ChevronRight } from 'lucide-react';
...
export default function NewsCard() {
  ...
  useEffect(() => {
    ...
    if (cached) {
      try { setArticles(JSON.parse(cached)); setLoading(false); } catch {}
    }
    fetchNews();
  }, []);
    ...
    <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl overflow-hidden">
          ...
          <button
            ...
            className={`shrink-0 text-xs px-3 py-1 rounded-full border transition-colors ${
              activeSource === src
                ...
                : 'border-[#2a2a2a] text-[#888] hover:border-[#f0a050] hover:text-[#f0a050]'
            }`}
          >
            {src}
          </button>
      ...
      <div className="divide-y divide-[#1a1a1a]">
        {loading ? (
          ...
          <div className="px-4 py-8 text-center text-[#555] text-sm">No articles found</div>
        ) : (
          filtered.slice(0, 8).map((article, i) => (
              ...
              className="flex items-start gap-3 px-4 py-3 hover:bg-[#1a1a1a] transition-colors active:bg-[#1a1a1a]"
            >
              {/* Text */}
      ...
      {!loading && filtered.length > 0 && (
        <div className="px-4 py-3 border-t border-[#1a1a1a]">
          <p className="text-[#555] text-[10px] text-center">
            Updated every 15 min · Tap any story to read
          </p>
        </div>
      )}
    </div>
