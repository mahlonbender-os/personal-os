...
// ─── VetCard — defined outside parent so its own useState is stable ───────────

function VetCard({ visit, onDelete }: { visit: VetVisit; onDelete: (id: string) => void }) {
...
export default function KnoxPage() {
  ...
  useEffect(() => {
    setAge(getAge(KNOX_BIRTHDAY)); // client-side only — avoids UTC mismatch
  ...
  function openVetModal() {
    ...
    setVType('Wellness Exam'); setVVet(''); setVNotes(''); setVCost(''); setVNext('');
  ...
  async function saveWeight() {
    ...
    try {
      ...
      if (res.ok) { setShowWeightModal(false); await fetchAll(); }
  ...
  async function saveVet() {
    ...
    try {
      const res = await fetch('/api/knox/vet', {
        ...
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          date: vDate, visit_type: vType,
          vet_name: vVet || null, notes: vNotes || null,
          cost: vCost !== '' ? vCost : null,
          next_visit_date: vNext || null,
        }),
      });
      if (res.ok) { setShowVetModal(false); await fetchAll(); }
  ...
  const nextVetDate = vetVisits
    ...
    .sort()[0];
  const upcomingVets = vetVisits.filter((v) => v.date > today).sort((a, b) => a.date.localeCompare(b.date));
  const pastVets = vetVisits.filter((v) => v.date <= today).sort((a, b) => b.date.localeCompare(a.date));
  ...
  return (
    <>
      <PullToRefresh onRefresh={fetchAll}>
        <div className="pb-24 min-h-screen bg-black">
          ...
          <div className="px-4 pt-6 pb-2">
            ...
            {age && <p className="text-[#555] text-sm mt-0.5">{age} · Siberian Husky</p>}
          </div>

          {/* Tab bar — manual, since SwipeTabs only handles swipe/slide */}
          <div className="flex border-b border-[#1a1a1a] px-4 mt-2">
            {TABS.map((tab) => (
              <button
                key={tab.id}
          ...
          {/* SwipeTabs — all 3 panels always rendered, sliding between them */}
          <SwipeTabs tabs={TABS} activeTab={activeTab} onTabChange={setActiveTab}>
            ...
            <div className="px-4 pt-4 space-y-4">
              <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-5">
                ...
                <div className="mt-5 pt-4 border-t border-[#1a1a1a] grid grid-cols-3">
              ...
              <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-5">
                <h3 className="text-xs text-[#555] font-semibold uppercase tracking-wider mb-3">Breed Info</h3>
                {[
              ...
              <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-5">
                <h3 className="text-xs text-[#555] font-semibold uppercase tracking-wider mb-3">Log Stats</h3>
                {[
                      ...
                      <div key={entry.id} className="bg-[#111] border border-[#1a1a1a] rounded-xl p-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            <p className="text-white font-mono font-semibold">{entry.weight_lbs} lbs</p>
                            {change !== null && (
                              <p className={`text-xs font-mono ${change > 0 ? 'text-green-400' : change < 0 ? 'text-red-400' : 'text-[#555]'}`}>
              ...
              {vetVisits.length === 0 ? (
                ...
                <div className="space-y-5">
                  {upcomingVets.length > 0 && (
                    <div>
                      <p className="text-xs text-[#f0a050] font-semibold uppercase tracking-wider mb-2">Upcoming</p>
                      <div className="space-y-2">
                        {upcomingVets.map((v) => <VetCard key={v.id} visit={v} onDelete={deleteVet} />)}
                      </div>
                    </div>
                  )}
                  {pastVets.length > 0 && (
                    <div>
                      <p className="text-xs text-[#555] font-semibold uppercase tracking-wider mb-2">Past Visits</p>
                      <div className="space-y-2">
                        {pastVets.map((v) => <VetCard key={v.id} visit={v} onDelete={deleteVet} />)}
                      </div>
                    </div>
                  )}
                </div>
      ...
      {/* FABs — outside PullToRefresh so they're fixed to viewport */}
      {activeTab === 'weight' && (
        <button
      ...
      {/* ── Log Weight Modal ─────────────────────────────────────────────────── */}
      {showWeightModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center px-4" onClick={() => setShowWeightModal(false)}>
          <div className="bg-[#1c1c1e] rounded-2xl w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className="text-[#555] text-xs uppercase tracking-wide block mb-1">Weight (lbs)</label>
                <input type="number" step="0.1" min="0" placeholder="e.g. 32.5" value={wLbs} onChange={(e) => setWLbs(e.target.value)}
                  className="w-full bg-[#111] border border-[#333] rounded-xl px-4 py-3 text-white text-sm" />
              </div>
              <div>
                <label className="text-[#555] text-xs uppercase tracking-wide block mb-1">Notes (optional)</label>
                <input type="text" placeholder="e.g. After morning meal" value={wNotes} onChange={(e) => setWNotes(e.target.value)}
                  className="w-full bg-[#111] border border-[#333] rounded-xl px-4 py-3 text-white text-sm" />
              </div>
            </div>
            <div className="flex gap-3 mt-6">
              <button onClick={() => setShowWeightModal(false)} className="flex-1 py-3 rounded-xl border border-[#333] text-[#888] text-sm">Cancel</button>
              <button onClick={saveWeight} disabled={savingW || !wLbs || !wDate}
      ...
      {/* ── Add Vet Visit Modal ──────────────────────────────────────────────── */}
      {showVetModal && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center px-4" onClick={() => setShowVetModal(false)}>
          <div className="bg-[#1c1c1e] rounded-2xl w-full max-w-sm max-h-[88vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="text" placeholder="e.g. Berks Animal Hospital" value={vVet} onChange={(e) => setVVet(e.target.value)}
                  className="w-full bg-[#111] border border-[#333] rounded-xl px-4 py-3 text-white text-sm" />
              </div>
              <div>
                <label className="text-[#555] text-xs uppercase tracking-wide block mb-1">Notes</label>
                <textarea placeholder="Vaccines given, medications, findings…" value={vNotes} onChange={(e) => setVNotes(e.target.value)}
                  rows={3} className="w-full bg-[#111] border border-[#333] rounded-xl px-4 py-3 text-white text-sm resize-none" />
              </div>
              <div>
                <label className="text-[#555] text-xs uppercase tracking-wide block mb-1">Cost ($)</label>
                <input type="number" step="0.01" min="0" placeholder="0.00" value={vCost} onChange={(e) => setVCost(e.target.value)}
                  className="w-full bg-[#111] border border-[#333] rounded-xl px-4 py-3 text-white text-sm" />
              </div>
              <div>
                <label className="text-[#555] text-xs uppercase tracking-wide block mb-1">Next Visit Date (optional)</label>
                <input type="date" value={vNext} onChange={(e) => setVNext(e.target.value)}
                  className="w-full bg-[#111] border border-[#333] rounded-xl px-4 py-3 text-white text-sm" />
              </div>
              <div className="flex gap-3 pt-2">
                <button onClick={() => setShowVetModal(false)} className="flex-1 py-3 rounded-xl border border-[#333] text-[#888] text-sm">Cancel</button>
                <button onClick={saveVet} disabled={savingV || !vDate || !vType}
                  className="flex-1 py-3 rounded-xl bg-[#f0a050] text-black text-sm font-semibold disabled:opacity-40">
                  {savingV ? 'Saving…' : 'Save'}
                </button>
              </div>
