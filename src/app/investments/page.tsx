...
export default function InvestmentsPage() {
  ...
  const [formDate, setFormDate] = useState(new Date().toLocaleDateString('sv-SE', { timeZone: 'America/New_York' }));
  ...
  // Auto-calculate shares when amount changes
  useEffect(() => {
  ...
  const gainLossColor = (n: number) => n >= 0 ? '#22c55e' : '#ef4444';
              ...
              {activeTab === 0 && (
                <>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {[{ symbol: 'VOO', label: 'S&P 500 ETF', price: data?.vooPrice }, { symbol: 'TSLA', label: 'Tesla', price: data?.tslaPrice }].map(t => (
                      <div key={t.symbol} className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-4">
                        ...
                        <div className="text-[#888] text-[10px] mb-2">{t.label}</div>
                  ...
                  {(data?.accounts || []).map((acc, i) => (
                    <div key={i} className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-4 space-y-3">
                          ...
                          {acc.holdings.map((h, j) => (
                            <div key={j} className="flex justify-between items-center bg-black/20 border border-[#151515] rounded-xl p-3">
                              <div>
                                ...
                                <div className="text-[#555] text-[10px] font-mono">@ {fmt(h.currentPrice)}</div>
                              </div>
                              <div className="text-right">
                                <div className="font-mono font-bold text-sm">{fmt(h.marketValue)}</div>
                                <div className="font-mono text-xs" style={{ color: gainLossColor(h.gainLoss) }}>
                      ...
                      {acc.holdings.length === 0 && (
                        <div className="text-[#333] text-xs font-mono text-center py-2">No positions logged yet</div>
                      )}
                    </div>
                  ))}
                </>
              ...
              {activeTab === 1 && (
                <>
                  {(!data?.trades || data.trades.length === 0) ? (
                    <div className="text-center py-16 text-[#333] text-sm">No trades logged yet</div>
                  ) : (
                    <div className="space-y-2">
                      {data.trades.map(trade => (
                        <div key={trade.id} className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-4 flex justify-between items-center">
                          <div>
                            <div className="flex items-center gap-2 mb-1">
                              <span className={`text-xs font-mono font-bold px-2 py-0.5 rounded ${trade.action === 'BUY' ? 'bg-[#22c55e]/10 text-[#22c55e]' : 'bg-[#ef4444]/10 text-[#ef4444]'}`}>
                                {trade.action}
                              </span>
                              <span className="text-[#f0a050] text-xs font-mono font-bold">{trade.security}</span>
                              <span className="text-[#555] text-[10px]">{trade.account}</span>
                            </div>
                            <div className="text-[#888] text-[10px] font-mono">{fmtDate(trade.date)}</div>
                            <div className="text-[#555] text-[10px] font-mono">{fmtShares(trade.shares)} shares</div>
                          </div>
                          <div className="flex items-center gap-3">
                            <span className="font-mono font-bold text-sm">{fmt(trade.amount)}</span>
                            <button
                              onClick={() => setDeleteId(trade.id)}
                              className="text-[#333] hover:text-[#ef4444] transition-colors text-lg leading-none"
                            >
                              ×
                            </button>
                          </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center px-4">
          <div className="bg-[#1c1c1e] rounded-2xl w-full max-w-md max-h-[85vh] overflow-y-auto pb-6">
            ...
            <div className="px-5 pt-4 space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    className="w-full bg-black border border-[#2a2a2a] p-3 rounded-xl text-white outline-none"
                  >
                  ...
                  {['BUY', 'SELL'].map(a => (
                    <button
                      ...
                      className={`py-3 rounded-xl text-xs font-bold border transition-colors ${
                        formAction === a
                          ? a === 'BUY' ? 'bg-[#22c55e]/20 border-[#22c55e] text-[#22c55e]' : 'bg-[#ef4444]/20 border-[#ef4444] text-[#ef4444]'
              ...
              <div>
                <div className="text-[#888] mb-1.5 uppercase tracking-wider">Shares (auto-calculated)</div>
                <input
                  type="number"
                  step="0.000001"
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <button
                  onClick={() => { setShowModal(false); setFormAmount(''); setFormShares(''); }}
                ...
                <button
                  ...
                  className="py-3 rounded-xl bg-[#f0a050] text-black text-xs font-bold disabled:opacity-50"
                >
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-end justify-center px-4 pb-8">
          <div className="bg-[#1c1c1e] rounded-2xl w-full max-w-md p-5 space-y-3">
            ...
            <div className="text-[#888] text-xs text-center">This cannot be undone. Your share count will update automatically.</div>
            <div className="grid grid-cols-2 gap-3 pt-1">
              <button onClick={() => setDeleteId(null)} className="py-3 rounded-xl bg-[#2a2a2a] text-white text-xs font-bold">
                Cancel
              </button>
              <button onClick={handleDelete} disabled={deleting} className="py-3 rounded-xl bg-[#ef4444] text-white text-xs font-bold disabled:opacity-50">
