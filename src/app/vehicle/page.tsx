...
const TABS = ['Overview', 'Maintenance', 'Mileage'];
...
export default function VehiclePage() {
  ...
  async function fetchAll() {
    setLoading(true);
    try {
      const [infoRes, maintRes, mileRes] = await Promise.all([
  ...
  const yearRecords = maintenance.filter(m => m.date.startsWith(thisYear.toString()));
  const yearCost = yearRecords.reduce((sum, m) => sum + (m.cost || 0), 0);
          ...
          {activeTab === 0 && (
            <div className="px-4 pt-2 space-y-3">
              ...
              <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-4">
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <p className="text-[#f0a050] text-xs font-semibold uppercase tracking-wider mb-1">
                      My Vehicle
                    </p>
                    {vehicleName ? (
                      <>
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-2">
                          {vehicleInfo?.color && (
                            <span className="text-[#ccc] text-sm">{vehicleInfo.color}</span>
                          )}
                          {vehicleInfo?.license_plate && (
                            <span className="text-[#ccc] text-sm font-mono tracking-wide">
                              {vehicleInfo.license_plate}
                            </span>
                          )}
                        </div>
                        {vehicleInfo?.vin && (
                          <p className="text-[#333] text-xs font-mono mt-1.5 break-all">
                            VIN: {vehicleInfo.vin}
                          </p>
                        )}
                      </>
                      ...
                      <p className="text-[#555] text-sm mt-1">
                        Tap Set Up to add your vehicle details
                      </p>
                    )}
                  </div>
                  <button
                    onClick={() => {
                      setVehicleForm(vehicleInfo || {});
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-4">
                  <p className="text-[#555] text-xs uppercase tracking-wide mb-1.5">
                    Current Mileage
                  </p>
                  ...
                  {latestMileage && (
                    <p className="text-[#555] text-xs mt-0.5">{formatDate(latestMileage.date)}</p>
                  )}
                </div>
                <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-4">
                  <p className="text-[#555] text-xs uppercase tracking-wide mb-1.5">
                    ...
                    <>
                      <p className="text-white text-2xl font-mono font-bold">
                        {lastOilChange.mileage
                          ? lastOilChange.mileage.toLocaleString()
              ...
              <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl overflow-hidden">
                ...
                {maintenance.length === 0 ? (
                  ...
                  maintenance.slice(0, 5).map((m, i) => (
                    <div
                      key={m.id}
                      className={`px-4 py-3 ${i < Math.min(maintenance.length, 5) - 1 ? 'border-b border-[#1a1a1a]' : ''}`}
                    >
                      <div className="flex items-center justify-between">
                        ...
                        {m.cost != null && m.cost > 0 ? (
                          ...
                          <p className="text-[#555] text-sm font-mono">Free</p>
                        ) : null}
                      </div>
                    </div>
                  ))
              ...
              {yearRecords.length > 0 && (
                <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-4">
                  <p className="text-[#555] text-xs uppercase tracking-wide mb-3">
                    {thisYear} Summary
                  </p>
                  <div className="flex justify-between items-end">
                    <div>
                      <p className="text-white text-2xl font-mono font-bold">{yearRecords.length}</p>
                      <p className="text-[#555] text-xs">Service records</p>
                    </div>
            ...
            <div className="px-4 pt-2 space-y-3">
              {loading && maintenance.length === 0 ? (
                <div className="space-y-3">
                  {[1, 2, 3].map(i => (
                    <div key={i} className="bg-[#111] border border-[#1a1a1a] rounded-2xl h-16 animate-pulse" />
                  ))}
                </div>
              ) : maintenance.length === 0 ? (
                  ...
                  <div key={m.id} className="bg-[#111] border border-[#1a1a1a] rounded-2xl overflow-hidden">
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <svg
                          className={`w-4 h-4 text-[#333] transition-transform duration-200 ${expandedMaint === m.id ? 'rotate-180' : ''}`}
                          fill="none"
                          stroke="currentColor"
                          viewBox="0 0 24 24"
                        >
                    ...
                    {expandedMaint === m.id && (
                      <div className="px-4 pb-4 pt-1 border-t border-[#1a1a1a] space-y-1.5">
                        {m.shop && (
                          <p className="text-[#ccc] text-sm">📍 {m.shop}</p>
                        )}
                        {m.notes && (
                          <p className="text-[#888] text-sm">{m.notes}</p>
                        )}
                        {!m.shop && !m.notes && (
                          <p className="text-[#333] text-sm">No additional details</p>
                        )}
                        <button
                          onClick={() => setShowDeleteMaint(m.id)}
          ...
          {activeTab === 2 && (
            <div className="px-4 pt-2 space-y-3">
              {loading && mileage.length === 0 ? (
                <div className="space-y-3">
                  {[1, 2, 3].map(i => (
                    <div key={i} className="bg-[#111] border border-[#1a1a1a] rounded-2xl h-20 animate-pulse" />
                  ))}
                </div>
              ) : mileage.length === 0 ? (
                <div className="bg-[#111] border border-[#1a1a1a] rounded-2xl p-8 text-center">
                        ...
                        <div className="flex-1">
                          ...
                          {diff !== null && diff > 0 && (
                            <p className="text-[#22c55e] text-xs mt-0.5">
                              +{diff.toLocaleString()} mi since previous entry
                            </p>
                          )}
                          {m.notes && (
                            <p className="text-[#888] text-xs mt-1">{m.notes}</p>
                          )}
                        </div>
                        <button
            ...
            <div className="bg-[#1c1c1e] rounded-2xl w-full max-h-[85vh] overflow-y-auto pb-6">
              <div className="px-5 pt-5 pb-3 flex items-center justify-between border-b border-[#2a2a2a]">
                ...
                <button
                  ...
                  className="text-[#555] text-2xl leading-none w-8 h-8 flex items-center justify-center"
                >
                  ✕
                </button>
              </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setVehicleForm(f => ({ ...f, year: parseInt(e.target.value) || undefined }))}
                      placeholder="2019"
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setVehicleForm(f => ({ ...f, trim_level: e.target.value }))}
                      placeholder="EX-L"
                      className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-[#f0a050]"
                    />
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setVehicleForm(f => ({ ...f, purchase_date: e.target.value }))}
                      className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-[#f0a050]"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setVehicleForm(f => ({ ...f, purchase_price: parseFloat(e.target.value) || undefined }))}
                      placeholder="25000"
                      className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-[#f0a050]"
                    />
                  </div>
              ...
              <div className="px-5 pt-4 space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={e => setMaintForm(f => ({ ...f, service_type: e.target.value }))}
                    className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-[#f0a050]"
                  >
                    {SERVICE_TYPES.map(t => (
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setMaintForm(f => ({ ...f, mileage: e.target.value }))}
                      placeholder="45231"
                      className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-white text-sm font-mono outline-none focus:border-[#f0a050]"
                    />
                  </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setMaintForm(f => ({ ...f, shop: e.target.value }))}
                    placeholder="Jiffy Lube, Dealer, DIY..."
                    className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-[#f0a050]"
                  />
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-white text-sm outline-none resize-none focus:border-[#f0a050]"
                  />
                </div>

                <button
                  onClick={saveMaintenance}
                  disabled={saving || !maintForm.date || !maintForm.service_type}
                  className="w-full bg-[#f0a050] text-black font-semibold py-3 rounded-xl disabled:opacity-50"
                >
              ...
              <div className="px-5 pt-4 space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={e => setMileageForm(f => ({ ...f, date: e.target.value }))}
                    className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-[#f0a050]"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    type="number"
                    value={mileageForm.odometer}
                    onChange={e => setMileageForm(f => ({ ...f, odometer: e.target.value }))}
                    placeholder="45231"
                    className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-white text-2xl font-mono outline-none focus:border-[#f0a050]"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="Before trip, after oil change..."
                    className="w-full bg-[#111] border border-[#2a2a2a] rounded-xl px-3 py-2.5 text-white text-sm outline-none focus:border-[#f0a050]"
                  />
                </div>
                ...
                <button
                  onClick={saveMileage}
                  disabled={saving || !mileageForm.date || !mileageForm.odometer}
                  className="w-full bg-[#f0a050] text-black font-semibold py-3 rounded-xl disabled:opacity-50"
                >
        ...
        {showDeleteMaint && (
          <div className="fixed inset-0 z-50 bg-black/70 flex items-end justify-center px-4 pb-8">
            <div className="bg-[#1c1c1e] rounded-2xl w-full p-5 space-y-3">
              ...
              <button
                onClick={() => deleteMaintenance(showDeleteMaint)}
                className="w-full bg-[#ef4444] text-white font-semibold py-3 rounded-xl"
              >
                Delete
              </button>
              <button
                onClick={() => setShowDeleteMaint(null)}
                className="w-full bg-[#1a1a1a] text-[#ccc] font-medium py-3 rounded-xl"
              >
                Cancel
              </button>
            </div>
        ...
        {showDeleteMileage && (
          <div className="fixed inset-0 z-50 bg-black/70 flex items-end justify-center px-4 pb-8">
            <div className="bg-[#1c1c1e] rounded-2xl w-full p-5 space-y-3">
              ...
              <p className="text-[#555] text-sm text-center">This cannot be undone.</p>
              <button
                onClick={() => deleteMileage(showDeleteMileage)}
                className="w-full bg-[#ef4444] text-white font-semibold py-3 rounded-xl"
              >
                Delete
              </button>
              <button
                onClick={() => setShowDeleteMileage(null)}
                className="w-full bg-[#1a1a1a] text-[#ccc] font-medium py-3 rounded-xl"
              >
                Cancel
              </button>
